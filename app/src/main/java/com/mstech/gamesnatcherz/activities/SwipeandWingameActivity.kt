...
import com.mstech.gamesnatcherz.adapter.SwipeandWinAdapter
import com.mstech.gamesnatcherz.utils.RetrofitApi
import kotlinx.android.synthetic.main.activity_gs_partners.*
import kotlinx.coroutines.launch

class SwipeandWingameActivity : AppCompatActivity() {
    var adapter: SwipeandWinAdapter? = null
    ...
    private suspend fun getRestaurentHistory() {
        ...
        val response = RetrofitApi().getSwipeandWin(SPStaticUtils.getString(SharedKey.CUSTOMER_ID,"0"))
        try {
            if(response.isSuccessful){
                var devicelist = response.body()
                var adapter: SwipeandWinAdapter? =
                    devicelist.let {
                        it?.let { it1 ->
                            SwipeandWinAdapter(
                                this,
