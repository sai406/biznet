...
import com.mstech.gamesnatcherz.adapter.TextQuizAdapt
import com.mstech.gamesnatcherz.utils.RetrofitApi
import kotlinx.android.synthetic.main.activity_gs_partners.*
import kotlinx.coroutines.launch

class TextQuizActivity : AppCompatActivity() {
    var adapte: TextQuizAdapt? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_text_quizz)
    ...
    private suspend fun getRestaurentHistory() {
        ...
        val response = RetrofitApi().getTextQuiz(SPStaticUtils.getString(SharedKey.CUSTOMER_ID,"0"))
        try {
            if(response.isSuccessful){
                var devicelist = response.body()
                var adapter: TextQuizAdapt? =
                    devicelist.let {
                        it?.let { it1 ->
                            TextQuizAdapt(
                               this,
