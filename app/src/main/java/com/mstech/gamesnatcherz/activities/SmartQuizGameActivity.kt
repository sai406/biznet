...
import com.mstech.gamesnatcherz.adapter.SmartQuizAdapter
import com.mstech.gamesnatcherz.utils.RetrofitApi
import kotlinx.android.synthetic.main.activity_gs_partners.*
import kotlinx.coroutines.launch

class SmartQuizGameActivity : AppCompatActivity() {
    var adapte: SmartQuizAdapter? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_smart_quiz_game)
    ...
    private suspend fun getRestaurentHistory() {
        ...
        val response = RetrofitApi().getSmartQuiz(SPStaticUtils.getString(SharedKey.CUSTOMER_ID,"0"))
        try {
            if(response.isSuccessful){
                var devicelist = response.body()
                var adapter: SmartQuizAdapter? =
                    devicelist.let {
                        it?.let { it1 ->
                            SmartQuizAdapter(
                                this,
